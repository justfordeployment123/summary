...
export default function AdminSettingsPage() {
    ...
    function switchSection(id: string) {
        ...
        // Reset cleanup when leaving ops tab
        if (activeSection === "ops") {
            setPreview(null); setResult(null);
            setCleanupError(null); setShowConfirm(false);
        }
        setAnimating(true);
        setTimeout(() => {
    ...
    async function saveSetting(key: keyof Settings, value: unknown) {
        ...
        try {
            ...
            setTimeout(() => setSaveStatus((s) => { const n = { ...s }; delete n[key]; return n; }), 2500);
        } catch {
    ...
    const SECTIONS: SettingSection[] = [
        {
            ...
            icon: Icons.ocr,
            fields: [
                { key: "ocr_confidence_flag_threshold",   label: "Flag Threshold",   type: "number", unit: "%", description: "Jobs below this confidence get a warning (default: 85%)" },
                { key: "ocr_confidence_reject_threshold", label: "Reject Threshold", type: "number", unit: "%", description: "Jobs below this are rejected and deleted (default: 70%)" },
            ],
        },
        {
            id: "openai",
            ...
            icon: Icons.ai,
            fields: [
                { key: "openai_model",                   label: "Model",             type: "select", options: ["gpt-4.1", "gpt-4.1-mini", "gpt-4o", "gpt-4-turbo"], description: "OpenAI model used for all generation tasks" },
                { key: "openai_input_word_limit",        label: "Input Word Cap",    type: "number", unit: "words",  description: "Hard cap on input text before sending to OpenAI (default: 1,200)" },
                { key: "openai_max_input_tokens",        label: "Max Input Tokens",  type: "number", unit: "tokens", description: "Per-request input token limit" },
                { key: "openai_max_output_tokens",       label: "Max Output Tokens", type: "number", unit: "tokens", description: "Per-request output token limit" },
                { key: "openai_monthly_token_cap",       label: "Monthly Token Cap", type: "number", unit: "tokens", description: "Auto-disables paid AI at 100% usage" },
                { key: "openai_monthly_cost_cap",        label: "Monthly Cost Cap",  type: "number", unit: "USD¢",   description: "Alternative cost-based monthly cap in cents" },
                { key: "openai_alert_threshold_percent", label: "Alert Threshold",   type: "number", unit: "%",      description: "Send email alert at this % of monthly cap (default: 80%)" },
            ],
        },
        {
            id: "files",
            ...
            icon: Icons.files,
            fields: [
                { key: "download_link_expiry_hours", label: "Download Link Expiry", type: "number", unit: "hours", description: "How long download links stay active after job completion (default: 72h)" },
                { key: "file_retention_hours",       label: "S3 File Retention",    type: "number", unit: "hours", description: "How long uploaded files remain in S3 before deletion (default: 24h)" },
            ],
        },
        {
            id: "disclaimer",
            ...
            fields: [
                ...
                { key: "disclaimer_checkbox_label", label: "Checkbox Label",  type: "text",     description: "Label for the mandatory acknowledgement checkbox" },
            ],
        },
        {
            id: "ops",
            ...
            icon: Icons.ops,
            fields: [
                { key: "stuck_job_timeout_minutes",   label: "Stuck Job Timeout",          type: "number", unit: "mins", description: "Flag jobs stuck in processing longer than this duration" },
                { key: "max_regeneration_attempts",   label: "Max Regeneration Attempts",  type: "number",               description: "Maximum times admin can regenerate a failed job (default: 3)" },
                { key: "job_metadata_retention_days", label: "Job Data Retention",         type: "number", unit: "days", description: "How long job records are kept before cleanup deletes them (default: 365 days = 12 months)" },
            ],
        },
    ];
                                    ...
                                    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                                        <div className="px-5 py-4 border-b border-slate-100">
                                            ...
                                            <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">
                                                Deletes job records older than the retention period set above, along with their associated payment, token, and webhook log records.
                                            </p>
                                        </div>
                                        ...
                                        <div className="px-5 py-4 space-y-4">
                                            ...
                                            {!preview && !result && (
                                                ...
                                                >
                                                    {loadingPreview ? (
                                                        <><span className="w-3 h-3 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" />Loading…</>
                                                    ) : (
                                                        <>
                                            ...
                                            {preview && !result && (
                                                <div className="space-y-3">
                                                    <div className={`rounded-xl px-4 py-3 border ${preview.expiredJobs === 0 ? "bg-slate-50 border-slate-200" : "bg-amber-50 border-amber-200"}`}>
                                                        ...
                                                        <p className="text-[11px] text-slate-500 font-mono">
                                                            Cutoff: {new Date(preview.cutoffDate).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        {showConfirm && (
                                                            <div className="flex items-center gap-3 w-full bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                                                                <p className="text-xs text-red-700 font-medium flex-1">This cannot be undone. Are you sure?</p>
                                                                <button
                                                                    onClick={runCleanup}
                                                                ...
                                                                <button onClick={() => setShowConfirm(false)} className="text-xs text-red-400 hover:text-red-600 shrink-0">Cancel</button>
                                                            </div>
                                                        )}

                                                        <button
                                                            onClick={() => { setPreview(null); setShowConfirm(false); }}
                                                    ...
                                                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-1.5 mt-2">
                                                            {[
                                                                ...
                                                                <div key={label} className="flex items-center justify-between bg-white rounded-lg px-3 py-1.5 border border-emerald-100">
                                                                    <span className="text-xs text-slate-500">{label}</span>
                                                                    <span className="text-xs font-bold font-mono text-emerald-700">{value.toLocaleString()}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                                            ...
                                            {cleanupError && (
                                                <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                                                    ...
                                                    <button onClick={() => setCleanupError(null)} className="text-red-400 hover:text-red-600 shrink-0">
...
// ─── Setting Card ─────────────────────────────────────────────────────────────
function SettingCard({ field, value, onChange, onSave, isSaving, status, animDelay }: {
    field: SettingField;
    value: any;
    ...
    const inputClass = "w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm text-slate-900 bg-white transition-all";
            ...
            <div className="px-5 py-4 flex items-end gap-3">
                <div className="flex-1">
                    {field.type === "textarea" ? (
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            />
                            {field.unit && (
                                <span className="text-xs font-semibold text-slate-400 shrink-0 bg-slate-100 px-2.5 py-1.5 rounded-lg">{field.unit}</span>
                            )}
                        </div>
                    )}
                </div>
                ...
                >
                    {isSaving
                        ...
                        : <><svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>Save</>
                    }
                </button>
            </div>
        </div>
