...
import {
  GlobalStyles, Navbar, HeroSection, HowItWorksSection,
  UploadSection, FAQSection, CTABanner, Footer,
  SummaryView, CompletedView,
...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [catRes, upsellRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { presignedUrl, s3Key, jobId, accessToken } = await requestUploadUrl({
        fileName: file.name, fileType: file.type, category: categoryId,
        firstName: firstName.trim(), email: email.trim(), marketingConsent,
      });

      setCurrentStep(1); setUploadStatus(PROCESS_STEPS[1]);
      await uploadFileToS3(presignedUrl, file);

      setCurrentStep(2); setUploadStatus(PROCESS_STEPS[2]);
      ...
      setCurrentStep(3); setUploadStatus(PROCESS_STEPS[3]);
      const aiResult = await generateFreeSummary({ jobId, extractedText: ocrResult.extractedText });

      setCurrentStep(4); setUploadStatus(PROCESS_STEPS[4]);
          ...
          {view === "form" && (
            <>
              ...
              <UploadSection
                ...
                uploadStatus={uploadStatus} isError={isError}
