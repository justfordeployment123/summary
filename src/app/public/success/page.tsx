...
function SuccessContent() {
    ...
    const pollJobStatus = useCallback(async () => {
        if (!jobId || !accessToken) {
            setError("Invalid link — missing job reference or access token. Please contact support if you have been charged.");
            return true; // Stop polling
        }

        try {
            // Securely poll the backend using the API utility
            const data = await checkJobStatus({ jobId, accessToken });
            ...
            if (data.status === "COMPLETED") {
                if (data.detailedBreakdown) {
                    ...
                    setError(
                        "Your breakdown completed but the content could not be loaded. Please contact support with your reference ID: " +
